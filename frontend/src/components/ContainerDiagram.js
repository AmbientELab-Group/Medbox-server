...
const Diagram = ({ container, pathProps, textProps, ...rest }) => {
    ...
    for (let i = 0; i < capacity; i++) {
        points.push([
            [
                Math.cos(ANGLE_ORIGIN + ANGLE_DELTA * i + PADDING / 2) *
                    SMALL_R,
                Math.sin(ANGLE_ORIGIN + ANGLE_DELTA * i + PADDING / 2) *
                    SMALL_R,
            ],
            [
                Math.cos(ANGLE_ORIGIN + ANGLE_DELTA * (i + 1) - PADDING / 2) *
                    SMALL_R,
                Math.sin(ANGLE_ORIGIN + ANGLE_DELTA * (i + 1) - PADDING / 2) *
                    SMALL_R,
            ],
            [
                Math.cos(ANGLE_ORIGIN + ANGLE_DELTA * (i + 1) - PADDING / 2) *
            ...
            [
                Math.cos(ANGLE_ORIGIN + ANGLE_DELTA * i + PADDING / 2) * BIG_R,
                Math.sin(ANGLE_ORIGIN + ANGLE_DELTA * i + PADDING / 2) * BIG_R,
            ],
            [
                Math.cos(
                    ANGLE_ORIGIN +
